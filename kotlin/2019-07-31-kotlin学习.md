...
lateinit标示着某个变量将会在稍后的操作中进行初始化，是用来对不可空的var变量）进行操作的，对val变量（无法进行操作
...
主构造方法是没有方法体的，它的方法操作放到了init函数中中
...
类内部的对象声明，没有被`inner` 修饰的内部类都是静态的，kotlin 默认都是
...
`companion object` 修饰为伴生对象,伴生对象在类中只能存在一个，大括号中都是静态的
...
所有静态方法（包括companion object中）无法调用外界非静态的成员
